...
const getEventStatus = (event: CalendarEvent): string => {
  // First, check if event has an explicit status from database
  if (event.status) {
    return event.status;
  }
...
const DashboardCalendar: React.FC = () => {
  ...
  const updateEventStatusInBackend = async (eventId: number, newStatus: string): Promise<void> => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api'}/calendar/events/${eventId}/status`, {
        method: 'PUT',
        headers: {
  ...
  const testBackendConnectivity = async (): Promise<boolean> => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api'}/actuator/health`, {
        method: 'GET',
  ...
  const handleCreateEvent = async (): Promise<void> => {
    ...
    try {
      ...
      const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api'}/calendar/events`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const handleUpdateEvent = async (): Promise<void> => {
    ...
    try {
      const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api'}/calendar/events/${selectedEvent.id}`,
          {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const handleDeleteEvent = async (eventId: number): Promise<void> => {
    ...
    try {
      const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api'}/calendar/events/${eventId}`,
          {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const connectWebSocket = useCallback((): (() => void) => {
    ...
    try {
      const client = new Client({
        webSocketFactory: () => new SockJS(`${import.meta.env.VITE_API_BASE_URL?.replace('/api', '') || 'http://localhost:8080'}/ws`),
            ...
            switch(data.action) {
              ...
              case "STATUS_UPDATE":
                if (data.eventId && data.status) {
                  setEvents(prev =>
                      prev.map(e =>
                          e.id === data.eventId ? { ...e, status: data.status as any } : e
                      )
                  );
                  if (selectedEvent?.id === data.eventId) {
                    setSelectedEvent(prev => prev ? { ...prev, status: data.status as any } : null);
                  }
                }
                break;
            }
  ...
  const fetchEvents = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      let response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api'}/calendar/events/company/${companyId}/month/${year}/${month}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        response = await fetch(
            `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api'}/calendar/events/company/${companyId}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
                ...
                <div className="space-y-4">
                  ...
                  {selectedEvent.allDay ? "All Day" :
                      ...
                      (selectedEvent.isEndDateOptional ? " (No end date)" :
                          " - " + formatEventDate(selectedEvent.endDate, true))
                  }
                </span>
                  </div>
