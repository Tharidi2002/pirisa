...
const Unit = () => {
    ...
    const getCompanyId = (): number => {
        ...
        return cmpnyId ? parseInt(cmpnyId) : 1;
    };
    ...
    // Delete unit with proper error handling
    const deleteUnit = async (id: number) => {
        try {
            ...
            if (axios.isAxiosError(err)) {
                ...
                const serverMessage = errorData?.resultDesc || errorData?.message || err.message;
                
                // Check for specific error patterns
                if (status === 500) {
                    // Foreign key constraint error
                    if (serverMessage?.toLowerCase().includes("foreign key") ||
                        serverMessage?.toLowerCase().includes("constraint") ||
                        serverMessage?.toLowerCase().includes("cannot delete") ||
                        serverMessage?.toLowerCase().includes("referenced")) {
                        errorMessage = "⚠️ Cannot delete this department because it has employees or designations assigned to it.\n\nPlease reassign or delete the associated records first.";
                        errorType = "constraint";
                    } else {
                        errorMessage = serverMessage || "Server error occurred. Please try again.";
                        errorType = "server";
                    }
                } else if (status === 400) {
                    errorMessage = serverMessage || "Bad request. Please check the department data.";
                    errorType = "bad_request";
                } else if (status === 404) {
                    errorMessage = "Department not found. It may have been already deleted.";
                    errorType = "not_found";
                } else {
                    errorMessage = serverMessage || `Error ${status}: Failed to delete department.`;
                    errorType = "general";
                }
    ...
    const handleDeleteUnit = async (id: number) => {
        const department = units.find(d => d.id === id);
        const designationCount = department?.designationList?.length || 0;

        let confirmMessage = "Are you sure you want to delete this department?";
        if (designationCount > 0) {
            confirmMessage = `⚠️ This department has ${designationCount} designation(s) assigned to it.\n\nDeleting it will also remove all associated designations.\n\nAre you sure you want to continue?`;
        }
        ...
        if (window.confirm(confirmMessage)) {
            ...
            if (!result.success) {
                // Build error message with actions
                let errorContent: React.ReactNode;
                
                if (result.errorType === "constraint") {
                    errorContent = (
                        <div className="space-y-3">
                            <div className="flex items-start gap-2">
                                <span className="text-red-500 text-lg">⚠️</span>
                                <p className="font-medium text-red-800 whitespace-pre-line">{result.message}</p>
                            </div>
                            <div className="flex flex-wrap gap-2 mt-2">
                                <button
                                    className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium transition-colors"
                                    onClick={() => {
                                        // Try to view employees page with department filter
                                        window.location.href = `/employee/all?department=${id}`;
                                    }}
                                >
                            ...
                            <p className="text-xs text-gray-500 mt-1">
                                💡 Tip: Make sure all employees and designations are removed from this department before deleting.
                            </p>
                        </div>
                    );
                } else {
    ...
    const addUnit = async (unitData: Partial<Unit>) => {
        try {
            ...
            if (response.status === 200) {
                ...
                const errorMsg = errorData.resultDesc || "Failed to add unit";
                setError(errorMsg);
            ...
            const errorMsg = "Failed to add unit. Please try again.";
            setError(errorMsg);
            toast.error(errorMsg);
            return false;
        }
    ...
    const updateUnit = async (unitData: Partial<Unit>) => {
        try {
            ...
            if (response.status === 200) {
                ...
                const errorData = response.data;
                const errorMsg = errorData.resultDesc || "Failed to update unit";
                setError(errorMsg);
                toast.error(errorMsg);
                return false;
            }
            ...
            const errorMsg = "Failed to update unit. Please try again.";
            setError(errorMsg);
            toast.error(errorMsg);
            return false;
        }
    ...
    const addDesignation = async (designationData: {
        ...
        try {
            ...
            const errorMsg = "Failed to add designation. Please try again.";
            setError(errorMsg);
            toast.error(errorMsg);
            return false;
        }
    ...
    const deleteDesignation = async (id: number) => {
        try {
            ...
            const errorMsg = "Failed to delete designation. Please try again.";
            setError(errorMsg);
            toast.error(errorMsg);
            return false;
        }
    ...
    const handleUnitSubmit = async (e: React.FormEvent) => {
        ...
        if (!currentUnit?.dpt_name?.trim() || !currentUnit?.dpt_code?.trim()) {
            const errorMsg = "Unit name and code are required";
            setError(errorMsg);
            toast.error(errorMsg);
            return;
        }
        ...
        if (currentUnit.id) {
            ...
            if (originalDept) {
                if (
                    ...
                    const errorMsg = "Unit code and name cannot be updated as they are professional identifiers";
                    setError(errorMsg);
                    toast.error(errorMsg);
                    return;
                }
        ...
        if (!currentUnit.id) {
            ...
            if (isDuplicate) {
                const errorMsg = "Unit code or name already exists";
                setError(errorMsg);
                toast.error(errorMsg);
                return;
            }
    ...
    const handleDesignationSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!currentDesignation.designation.trim()) {
            const errorMsg = "Designation name is required";
            setError(errorMsg);
            toast.error(errorMsg);
            return;
        }
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-3 sm:p-4 lg:p-6">
            ...
            <div className="max-w-7xl mx-auto">
                ...
                {/* Error Alert */}
                {error && (
                    <div className="mb-4 sm:mb-6 bg-red-50 border-l-4 border-red-500 p-4 sm:p-5 rounded-lg shadow-sm">
                        <div className="flex items-start">
                            ...
                            <div className="ml-3 flex-1">
                                {typeof error === 'string' ? (
                                    <p className="text-sm text-red-700 break-words whitespace-pre-line">{error}</p>
                                ) : (
                                    error
                                )}
                            </div>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6">
                        {filteredUnits.map((dept) => (
                            ...
                            >
                                ...
                                <div className="p-4 sm:p-6" id={`designations-${dept.id}`}>
